...
void Stage_DM()
{
    ...
    /* Register Value */
    DM_Forward_data = DM_ALU_outcome = EX_ALU_outcome;
    DM_HI = HI.cur;
    DM_LO = LO.cur;
    DM_PC = EX_PC;
    ...
    else if ( Inst_DM == "MFHI" )
        DM_Forward_data = HI.cur;
    else if ( Inst_DM == "MFLO" )
        DM_Forward_data = LO.cur;
}
...
void Stage_EX()
{
    ...
    // if(cyc==40) cout << " DM_ALU_outcome : "<< setw(8) << setfill('0') << hex << DM_ALU_outcome << endl;
         ...
         Inst_DM == "JAL")
    {
            ...
            Inst_EX == "SW"   || Inst_EX == "SH"   || Inst_EX == "SB"  )
        {
            ...
            {
                isRSForwarded = true;
                ALU_rs_value = DM_Forward_data;
                EX_sinfo = EX_sinfo + " fwd_EX-DM_rs_$" + srs.str();
            }
            if( DM_rd == rt && DM_rd != 0 )
            {
                isRTForwarded = true;
                ALU_rt_value = DM_Forward_data;
